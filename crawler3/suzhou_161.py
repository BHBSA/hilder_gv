    ...
    def start_crawler(self):
        ...
        for region in ['工业园区','吴中区','高新区','相城区','姑苏区','吴江区']:
            ...
            formdata = {'__VIEWSTATE':viewstate,
                        ...
                        'ctl00$MainContent$ddl_RD_CODE':rd_code,
                        'ctl00$MainContent$bt_select':select}
            index_res = requests.post(self.start_url,data=formdata)
            ...
            for i in range(1,int(page)+1):
                if i==1:
                    ...
                    page_res = requests.post(self.start_url,data=data)
                    ...
                    for url in url_list[:9]:
                        co_url = 'http://spf.szfcweb.com/szfcweb/(S(hdeijxnxs01rlpun22pnfha0))/DataSerach/'+url
                        co_res = requests.get(co_url,headers=self.headers)
                        co = Comm(co_index)
                        co.co_name = re.search('项目名称.*?NAME">(.*?)</span',co_res.text).group(1)
                        co.co_id = re.search('ID=(.*)',url).group(1)
                        co.co_develops = re.search('公司名称.*?Com">(.*?)</span',co_res.text).group(1)
                        co.co_address = re.search('项目地址.*?Add">(.*?)</span',co_res.text).group(1)
                        co.insert_db()
                        bu_list = page_html.xpath("//table[@id='ctl00_MainContent_OraclePager1']/tbody/tr")
                        self.bu_parse(co.co_id,bu_list)

    def bu_parse(self,co_id,bu_list):
        for bu in bu_list[1:]:
            build = Building(co_index)
            build.co_id = co_id
            build.bu_id = bu.xpath("//form/@action")[0]
            build.bu_num = bu.xpath("./td/a/text()")[0]
            build.bu_build_size = bu.xpath("./td[1]/text()")[0]
            build.bu_floor = bu.xpath("./td[3]/text()")[0]
            build.bu_all_house = bu.xpath("./td[2]/text()")[0]
            build.insert_db()
            tmp_url = bu.xpath("./td/a/@href")[0]
            house_url = 'http://spf.szfcweb.com/szfcweb/(S(hdeijxnxs01rlpun22pnfha0))/DataSerach/'+tmp_url
