...
class Baise(Crawler):
    ...
    def get_comm_detail(self, href, comm):
        ...
        co_volumetric = re.search(
            r'容积率：</td><tdalign="left"class="padingleft3px">(.*)?</td><tdalign="right"><palign="left">绿化率', html).group(
            1)  # 容积率
        co_green = re.search(
    ...
    def get_build_detail(self, building_url, building, co_id , CO_INDEX):
        ...
        bu_all_house = re.search(
            r'总&nbsp;套&nbsp;数：</td><tdalign="left"class="padingleft3px">(.*?)</td><tdalign="right">可售套数', html).group(1) # 总套数
        bu_floor = re.search(r'总层数：</td><tdalign="left"class="padingleft3px">(.*)?</td><tdalign="right">项目类型', html).group(1)  # 总层数
        bu_build_size = re.search(
            r'建筑面积：</td><tdalign="left"class="padingleft3px"><FONTcolor=#ff0000>(.*)?M&sup2;</FONT></td><tdalign="right">住宅面积'
            , html).group(1)  # 建筑面积
        bu_live_size = re.search(r'住宅面积：</td><tdalign="left"class="padingleft3px">(.*)?M&sup2;</td></tr><tr><tdheight="25"align="right">幢套内建筑面积', html).group(1)  # 住宅面积
        bu_not_live_size = re.search(r'非住宅面积：</td><tdalign="left"class="padingleft3px">(.*)?M&sup2;</td></tr><tr><tdheight="25"align="right">预'
                                     , html).group(1)  # 非住宅面积
        bu_pre_sale = re.search(r'售许可证：</td><tdalign="left"class="padingleft3px">(.*?)</td>', html).group(1)    # 楼栋预售证书
